...
class RepositorioOrgao_Gestor extends Repositorio {
    ...
    function buscarEmails($VO) {
        ...
        $query = "SELECT * FROM EMAIL_GESTOR_ESTAGIO
                  WHERE ID_ORGAO_GESTOR_ESTAGIO = '" . $VO->ID_ORGAO_GESTOR_ESTAGIO . "'";
        // print_r($query);
        return $this->sqlVetor($query);
    }
    ...
    function inserirEmail($VO) {
        ...
        $queryPK = "select SEMAD.F_G_PK_EMAIL_GESTOR_ESTAGIO('" . $VO->ID_ORGAO_GESTOR_ESTAGIO . "') as NB_EMAIL_GESTOR_ESTAGIO from DUAL";
        $this->sqlVetor($queryPK);
        $CodigoPK = $this->getVetor();

        $query = "
            INSERT INTO EMAIL_GESTOR_ESTAGIO
            (NB_EMAIL_GESTOR_ESTAGIO,
            ID_ORGAO_GESTOR_ESTAGIO,
            TX_EMAIL)
            VALUES('" . $CodigoPK['NB_EMAIL_GESTOR_ESTAGIO'][0] . "',
            '" . $VO->ID_ORGAO_GESTOR_ESTAGIO . "',
            '" . mb_strtolower($VO->TX_EMAIL) . "')";

//        $retorno = $this->sql($query);
//        return $retorno ? '' : $CodigoPK['NB_EMAIL_GESTOR_ESTAGIO'][0];
        return $this->sql($query);
    }
    ...
    function excluirEmail($VO) {
        ...
        $query = "
            delete from EMAIL_GESTOR_ESTAGIO
                where ID_ORGAO_GESTOR_ESTAGIO = '" . $VO->ID_ORGAO_GESTOR_ESTAGIO . "'
                  and NB_EMAIL_GESTOR_ESTAGIO = '" . $VO->NB_EMAIL_GESTOR_ESTAGIO . "'";
        //print_r($query);
        return $this->sql($query);
    }
