...
class RepositorioOferta_vaga extends Repositorio {
...
//print_r($query); die;
    ...
    function pesquisarOrgaoSolicitante($VO) {
        $query = "
        select distinct orgao_estagio.id_orgao_estagio CODIGO,
		 	orgao_estagio.tx_orgao_estagio 
         	from agente_setorial_estagio, 
         	orgao_agente_setorial,
			orgao_estagio
			where (agente_setorial_estagio.id_setorial_estagio = orgao_agente_setorial.id_setorial_estagio)
			and (orgao_agente_setorial.ID_ORGAO_ESTAGIO = orgao_estagio.ID_ORGAO_ESTAGIO)
			and (agente_setorial_estagio.id_usuario = '".$_SESSION['ID_USUARIO']."')
			and (agente_setorial_estagio.id_setorial_estagio = '".$_SESSION['ID_SETORIAL_ESTAGIO']."')
			and orgao_estagio.CS_STATUS = 1;	
        ";
...
dd($query);die;
	...
	function buscarTipoVaga($VO) {
        $query = "select B.CS_TIPO_VAGA_ESTAGIO CODIGO, C.TX_TIPO_VAGA_ESTAGIO
...
$VO->CS_SITUACAO? $query .= "CS_SITUACAO        = '" . $VO->CS_SITUACAO . "'," : false;
	...
	function pesquisarValorBolsa($VO) {
        $query = "select ID_BOLSA_ESTAGIO CODIGO, to_char(NB_VALOR, 'FM999G999G999D90') NB_VALOR from BOLSA_ESTAGIO";
